...
public class PublicLiabilityDetailsStepDefinition {

	TestContextSetup _testContextSetup;
	PublicLiabilityDetails _publicLiabilityDetails;
	String _currentUrl = "";

	List<Quintet<String, String, String, String, String>> _resultReport = new ArrayList<Quintet<String, String, String, String, String>>();


	public PublicLiabilityDetailsStepDefinition(TestContextSetup testContextSetup) {
		this._testContextSetup = testContextSetup;
		this._publicLiabilityDetails = _testContextSetup.ObjPageObjectManager.GetPublicLiabilityDetailsPage();
	}

	@Given("PublicLiability Launch Browser and URL {string}")
	...
	@And("PublicLiability Execute Userinfo Page")
...
//					_publicLiabilityDetails = _testContextSetup.ObjPageObjectManager.GetPublicLiabilityDetailsPage();
...
//				_resultReport = null;
	...
	public void PublishReports() {
		...
		if (createExcelFlag.toLowerCase().equals("true"))
			_testContextSetup.ObjCommonFunctionLib.PublishReport(_resultReport, sanityReportsColumnsName, "PublicLiability");
	}
	...
	private void ControlsProcessor(List<Map<String, String>> listData, int counter) {
		...
		try {
			for (Map.Entry<String, String> cellData : listData.get(counter).entrySet()) {
				...
				//if (!controlValue.toLowerCase().equals("pass") & !controlValue.toLowerCase().equals("fail")) {
					if (_publicLiabilityDetails.controlInfo.containsKey(cellData.getKey())) {
						var controlInfo = _publicLiabilityDetails.controlInfo.get(cellData.getKey());
						elementActionStatus = _publicLiabilityDetails.AssignControl(controlInfo.getValue0(), controlInfo.getValue1(), controlInfo.getValue2(), controlValue);
...
//				}
//				else {
//					elementActionStatus = controlValue;
//					controlValue = cellData.getValue();
//				}
